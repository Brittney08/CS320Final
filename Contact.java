...
public class Contact {
	...
	private static final String INITIALIZER_NUM = "1238888888"
	...
	protected void updatePhoneNumber(String phoneNumber) {
		...
		if (phoneNumber == null) {
			...
			throw new IllegalArgumentException("Phone number length invalid. Must be " + CONTACT_PHONENUM_LENGTH + "digits");
		}
		else if (!phoneNumber.matches(regex)) {
			thrwo new IllegalArgumentException("Phone number can only contain digits");
		}
		else {
	...
	protected void updateAddress(String address) {
		if (address == null) {
			...
			throw new IllegalArgumentException("Address must not be longer than " + CONTACT_ADDRESS_LENGTH + " characters");
		}
		else {
